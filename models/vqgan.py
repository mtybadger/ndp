...
class IBQSharedModel(pl.LightningModule):
    ...
    def forward(self, input):
        ...
        zero_1, zero_2, zero_3, zero_4 = self.get_zero_tokens(self.quantize.n_embed, self.embed_dim, input.device)
...
class VQSharedModel(pl.LightningModule):
    ...
    def init_from_ckpt(self, path, ignore_keys=list()):
        ...
        shape_mismatches = []
        for k in model_keys.intersection(sd_keys):
            if model_state[k].shape != sd[k].shape:
                shape_mismatches.append(
                    f"{k}: model shape {model_state[k].shape} vs loaded shape {sd[k].shape}"
                )
    ...
    def forward(self, input):
        ...
        zero_1, zero_2, zero_3, zero_4 = self.get_zero_tokens(self.quantize.n_e, self.embed_dim, input.device)
